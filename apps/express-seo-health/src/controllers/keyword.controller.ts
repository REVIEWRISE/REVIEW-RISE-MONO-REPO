...
export class KeywordController {
  ...
  async listKeywords(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.json(createSuccessResponse(resultData, 'Keywords listed successfully', 200, { requestId }));
    } catch (error) {
      console.error('Error listing keywords:', error);
      res.status(500).json(createErrorResponse('Failed to list keywords', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
  ...
  async createKeyword(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!data.businessId || !data.keyword) {
        res.status(400).json(createErrorResponse('businessId and keyword are required', ErrorCode.BAD_REQUEST, 400, undefined, requestId));
        return;
      }
      ...
      res.status(201).json(createSuccessResponse(keyword, 'Keyword created successfully', 201, { requestId }));
    } catch (error) {
      console.error('Error creating keyword:', error);
      res.status(500).json(createErrorResponse('Failed to create keyword', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
  ...
  async updateKeyword(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.json(createSuccessResponse(keyword, 'Keyword updated successfully', 200, { requestId }));
    } catch (error) {
      console.error('Error updating keyword:', error);
      res.status(500).json(createErrorResponse('Failed to update keyword', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
  ...
  async deleteKeyword(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.json(createSuccessResponse({ message: 'Keyword deleted successfully' }, 'Keyword deleted successfully', 200, { requestId }));
    } catch (error) {
      console.error('Error deleting keyword:', error);
      res.status(500).json(createErrorResponse('Failed to delete keyword', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
  ...
  async getKeywordRanks(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.json(createSuccessResponse(resultRanks, 'Keyword ranks fetched successfully', 200, { requestId }));
    } catch (error) {
      console.error('Error fetching keyword ranks:', error);
      res.status(500).json(createErrorResponse('Failed to fetch keyword ranks', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
 ...
 * POST /api/keywords/bulk - Bulk create keywords
  ...
  async bulkCreateKeywords(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!Array.isArray(keywords) || keywords.length === 0) {
        res.status(400).json(createErrorResponse('keywords array is required', ErrorCode.BAD_REQUEST, 400, undefined, requestId));
        return;
      }
      ...
      res.status(201).json(
        ...
        }, 'Bulk creation completed', 201, { requestId })
      );
    } catch (error) {
      console.error('Error bulk creating keywords:', error);
      res.status(500).json(createErrorResponse('Failed to bulk create keywords', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
  ...
  async suggestKeywords(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.json(createSuccessResponse({ suggestions: suggestions.slice(0, limit) }, 'Suggestions generated successfully', 200, { requestId }));
    } catch (error) {
      console.error('Error suggesting keywords:', error);
      res.status(500).json(createErrorResponse('Failed to suggest keywords', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
  ...
  async harvestCompetitor(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!data.businessId || (!data.competitorDomain && !data.competitorGbpId)) {
        res.status(400).json(createErrorResponse('businessId and competitorDomain or competitorGbpId are required', ErrorCode.BAD_REQUEST, 400, undefined, requestId));
        return;
      }
      const brand =
      ...
      res.json(createSuccessResponse({ keywords: harvested.slice(0, data.limit || 20) }, 'Keywords harvested successfully', 200, { requestId }));
    } catch (error) {
      console.error('Error harvesting competitor keywords:', error);
      res.status(500).json(createErrorResponse('Failed to harvest competitor keywords', ErrorCode.INTERNAL_SERVER_ERROR, 500, undefined, requestId));
    }
  }
}
