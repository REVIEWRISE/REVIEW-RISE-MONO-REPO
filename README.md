...
| **API Contracts** | `@platform/contracts` | Shared TypeScript source of truth for API Requests and Responses, ensuring strong typing across services. |
...
2. **Start Infrastructure & Setup Environment**
   ```bash
   pnpm run setup:app
   ```
   *This command copies `.env.example` to `.env` for all workspaces and launches PostgreSQL/Redis via Docker. The database is automatically initialized definition `.packages/@platform/db/scripts/init-db.sh`.*
...
| `express-social` | Social Media Content Scheduling & Posting |
| `express-gbp-rocket` | Google Business Profile Optimization & Sync |
| `express-ad-rise` | Ad Campaign Management |
| `express-brand` | Brand Identity & Asset Management |
| `express-admin-portal` | Internal Tools & Super Admin Dashboard |
...
| `@platform/contracts` | **Standardized API Types**: `ApiRequest`, `ApiResponse`, and query interfaces |
| `@platform/types` | Shared Domain DTOs and Enums |
| `@platform/auth` | JWT Validation, RBAC, and Async Local Storage setup for multi-tenancy |
| `@platform/queues` | Standardized BullMQ queue definitions and event definitions |
| `@platform/utils` | Shared utility functions (logging, date formatting, etc.) |
...
| `pnpm run setup:app` | Quickstart: Copies envs and starts Docker infra |
