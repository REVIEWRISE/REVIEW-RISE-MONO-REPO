...
async function seedBrandRiseData() {
  ...
  let location = await prisma2.location.findUnique({ where: { id: TECH_CAFE_LOCATION_ID } });
  if (!location) {
    console.log("Location not found, creating Tech Cafe Main location...");
    location = await prisma2.location.create({
      data: {
    ...
    await prisma2.location.update({
      where: { id: TECH_CAFE_LOCATION_ID },
      data: { businessId }
    });
  }
  console.log("\u{1F9EC} Seeding Brand DNA...");
  await prisma2.brandDNA.upsert({
    where: { businessId },
    update: {},
    create: {
      businessId,
  ...
  console.log("\u{1F3E2} Seeding Brand Profile...");
  ...
  const existingProfile = await prisma2.brandProfile.findFirst({
    where: { businessId, websiteUrl: "https://techcafe.io" }
  });
  if (existingProfile) {
    await prisma2.brandProfile.update({
      where: { id: existingProfile.id },
      data: brandProfileData
    });
  } else {
    await prisma2.brandProfile.create({
      data: brandProfileData
    });
  }
  ...
  const competitorsData = [
    {
      ...
      snapshot: {
        ...
        trustSignals: { badges: ["#1 Coffee Chain"], certifications: ["Fair Trade Certified"], reviewCount: 5e4, avgRating: 4.2 },
    ...
    },
    {
      ...
      snapshot: {
        ...
        trustSignals: { badges: ["Founded 1966", "Small Batch"], certifications: ["Organic Options", "Rainforest Alliance"], reviewCount: 15e3, avgRating: 4.5 },
    ...
    },
    {
      ...
      snapshot: {
        ...
        trustSignals: { badges: ["Over 200M reviews"], certifications: [], reviewCount: 2e8, avgRating: null },
  ...
  for (const comp of competitorsData) {
    const existingCompetitor = await prisma2.competitor.findFirst({
      where: {
        businessId,
        OR: [
          { domain: comp.domain },
    ...
    if (existingCompetitor) {
      competitor = await prisma2.competitor.update({
        where: { id: existingCompetitor.id },
        data: {
          name: comp.name,
          // Ensure name aligns with seed data
          domain: comp.domain,
      ...
      competitor = await prisma2.competitor.create({
        data: {
          businessId,
          name: comp.name,
          domain: comp.domain,
    ...
    await prisma2.competitorSnapshot.deleteMany({
      where: { competitorId: competitor.id }
    });
    await prisma2.competitorSnapshot.create({
      data: {
        ...
        metrics: {
          ...
          socialFollowers: Math.floor(Math.random() * 1e4) + 5e3,
  ...
  console.log("\u{1F4A1} Seeding Content Ideas...");
  const ideas = [
  ...
  for (const idea of ideas) {
    await prisma2.contentIdea.create({
      data: {
        businessId,
        title: idea.title,
  ...
  console.log("\u{1F4CA} Seeding Reports...");
  for (let i = 1; i <= 3; i++) {
    await prisma2.report.create({
      data: {
        businessId,
        title: `Monthly Performance Report - ${new Date(Date.now() - i * 30 * 864e5).toLocaleString("default", { month: "long" })}`,
        version: "v1.0",
        htmlContent: "<h1>Performance Report</h1><p>This is a generated report.</p>",
        generatedAt: new Date(Date.now() - i * 30 * 864e5)
      }
    });
  }
  console.log("\u{1F4C8} Seeding Visibility Snapshots...");
  const today = /* @__PURE__ */ new Date();
  for (let i = 29; i >= 0; i--) {
    const date = new Date(today);
    date.setDate(date.getDate() - i);
    await prisma2.visibilitySnapshot.create({
      data: {
        businessId,
        range: "30d",
  ...
  console.log("\u{1F916} Seeding AI Visibility Metrics...");
  const aiMetric = await prisma2.aIVisibilityMetric.create({
    data: {
      businessId,
      periodStart: new Date((/* @__PURE__ */ new Date()).setDate((/* @__PURE__ */ new Date()).getDate() - 7)),
  ...
  for (const p of aiPlatforms) {
    await prisma2.aIPlatformData.create({
      data: {
