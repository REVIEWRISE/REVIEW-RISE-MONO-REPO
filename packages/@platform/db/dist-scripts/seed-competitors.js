...
async function seedCompetitorData() {
  ...
  console.log("\u{1F331} Seeding Competitor data...");
  try {
    const business = await prisma2.business.findUnique({
    ...
    const competitorsData = [
      ...
      },
      {
        ...
        name: "Domino's",
        type: "AGGREGATOR",
        // Technically a chain, but acting as a major external force
    ...
    for (const data of competitorsData) {
      const competitor = await prisma2.competitor.upsert({
        where: {
          businessId_domain: {
    ...
    if (joeys) {
      await prisma2.competitorSnapshot.create({
        data: {
          ...
          serviceList: ["Dine-in", "Takeout", "Delivery", "Catering"],
          // Renamed from services
          pricingCues: ["$$"],
          // Mapped pricing to pricingCues array or similar logic
          trustSignals: { badges: ["4.5 stars on Google"], certifications: ["Family owned since 1990"] },
          // structured as JSON
    ...
    await prisma2.opportunitiesReport.create({
      data: {
        businessId: business.id,
