...
async function main() {
  ...
  console.log("\u{1F331} Starting review seed...");
  let locations;
  try {
    console.log("Fetching locations...");
    locations = await prisma2.location.findMany();
  } catch (e) {
      ...
      try {
        await prisma2.review.create({
          data: {
            ...
            platform,
            externalId: `seed-${location.id}-${i}-${Date.now()}-${Math.floor(Math.random() * 1e3)}`,
            author: `User ${Math.floor(Math.random() * 1e3)}`,
